...
Ptr<WifiMacQueue> GetQueue(Ptr<Node> node) //packet queue for each node
{
...
*/ 
Ptr<OpenGymDataContainer> MyGetObservation(void) // num of packet queue in each node
{
  uint32_t nodeNum = NodeList::GetNNodes ();
  std::vector<uint32_t> shape = {nodeNum,};
...
*/
float MyGetReward(void)  // receive packet num 
{
...
main (int argc, char *argv[])
{
  ...
  AnimationInterface anim("mesh-sim-qlearn2.xml");
